...
<head>
<meta charset="utf-8">
<title>${videoParent.video_parent_name}-涂涂影院-总有刁民想害朕</title>
...
<body>
	...
	<div style="width:88%; margin: 20px auto;height:550px;">
		<div class="block" style="background-color:#1a1a1a;border-radius: 6px 6px 6px 6px;">
			<!--正文内容-->
			<div class="main-0" style="padding: 0px;height:560px;">
				<div style="display: flex;height:520px;">
					<div class="player" style="width:90%;height:520px;">
						<h2 id="video_title" style="color: #fff;padding-left: 20px;">
							<div class="animated bounce">${videoParent.video_parent_name}</div>
						</h2>
						<div id="player" style="height:510px;margin-right:0px;">
							
						</div>
					</div>
					<div class="play-list" style="background: none;">
						<h2 style="color: #fff;">播放列表</h2>
						<ul id="play-list" class="play-list-ul">
							<c:forEach items="${videoChildrenList}" var="entity" varStatus="status" >
								
									<li id="${status.index}"  class="v-li drama 
										<c:if test="${selectId == fn:trim(status.index)}">
											 selected
										</c:if>">
										<a href="tvItemPlay?tv_id=${entity.video_parent_id}&selectId=${status.index}&tv_item_id=${entity.video_child_id}" class="drama-item">${entity.video_child_name}</a>
									</li>
					...
					<div class="main-20" style="margin-bottom: -20px;">
						<div class="grid-3-20 spacing-20">
							<div style="min-width: 140px;height: 210px;">
								<img id="haibao" src="${videoParent.photo}" width="100%" height="100%" />
							</div>
							<div style="color: #666;position: relative;">
								<h1><a id="biaoti" href="javascript:;">${videoParent.video_parent_name}</a></h1>
								
								<ul class="equal-3" style="margin-top: 20px;">
									<li id="diqu">地区：${videoParent.area_name}剧</li>
									<li id="nianfen">年份：${videoParent.time_name}</li>
									<li id="zhuangtai">状态：连载中</li>
									<span class="clearfix"></span>
								</ul>
								
								<p id="jianjie" style="margin-top: 10px;">
									简介：${videoParent.introduce}
	...
	javaex.goTopBtn({
		...
		<script type="text/javascript">
			var videoObject = {
					...
					drag: 'start', //拖动的属性
					video:'${play_url}'//视频地址
					//video:'http://youku.cdn-iqiyi.com/20180425/9917_49709613/index.m3u8'//视频地址
			};
